...
test.describe('Parent Data Display', () => {
  ...
  test('Tiger should see urgent renewal warning with only 2 classes left', async ({ page }) => {
    const { email, password, name, classesRemaining } = PARENT_ACCOUNTS.tiger
    ...
    // Should see urgent warning for low class count
    await expect(page.locator('text=Only 2 classes left').first()).toBeVisible({ timeout: 10000 })
    
    // Should see Tiger's class name
    await expect(page.locator('text=Banana').first()).toBeVisible()
  })

  test('Hardy should see Hardy 1v1 class with 21 classes remaining', async ({ page }) => {
    const { email, password, classesRemaining } = PARENT_ACCOUNTS.hardy
    ...
    // Should see 21 classes remaining
    await expect(page.locator(`text=${classesRemaining} classes remaining`).first()).toBeVisible()
  })
  ...
  test('Multiple parents should see their own data only', async ({ page }) => {
    // Login as Joy
    ...
    await expect(page.locator('text=Avocado')).not.toBeVisible()
  })
})
